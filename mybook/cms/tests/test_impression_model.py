...
class BookImpressionTests(TestCase):
    ...
    def test_impression_size_equals_book_size(self):
        ...
        self.assertEqual(saved_books.count(), saved_impressions.count())
    ...
    def test_impression_comment(self):
        comment = 'The book is Great'
        self.create_impression(comment)
        saved_impressions = Impression.objects.all()
        impression = saved_impressions[0]

        self.assertEqual(str(impression), comment)
